    ...
    public CreateOrder(List<Product> products, int customer){
        ...
        try {
            ...
            for(int i = 0; i< products.size(); i++) {
                stmt.execute("INSERT INTO ordered_product(name, orders_id, quantity, sell_price, brand) VALUES('" + products.get(i).getName() + "', " + orderId + ", '" + products.get(i).getChange() + "', " + products.get(i).getUnitPrice() + ", '" + products.get(i).getBrand() + "');");
